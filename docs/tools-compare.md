...
Claude 與 ChatGPT／Codex 加起來有五種以上的介面，名字又常常互相借用（Cowork、Work、Codex 分頁、Codex App……），第一次接觸很容易搞錯。這頁把八種常見介面放進同一張表對照，再挑出最容易搞混的五組單獨說明。每一格背後都有來源，標記沿用查證文件的原始編號（`S`＝Claude 官方來源、`O`＝ChatGPT／Codex 官方來源、`X`＝補充查證來源），對照表列在頁尾；查無明確資料的格子直接寫「查無」，不用推測填補。

## 主表：介面 × 方案

「✅」可用、「❌」不可用、「⚠️」有限制或需額外條件、「？」查無或官方文件互相矛盾。

| 介面 | 跑在哪裡 | 讀寫本機檔案 | 執行指令／程式碼 | Skill／Plugin／MCP | 最低方案 | 適合做什麼 |
|---|---|---|---|---|---|---|
| **Claude Chat**（網頁／手機） | Anthropic 雲端伺服器 [S7] | ❌ 不能 [S7] | ⚠️ 只有雲端的 code execution 沙盒工具，不是本機終端 [S7] | Skill✅／Plugin⚠️（付費方案可在網頁版 Chat 使用，但官方文件說法不一致，見[擴充機制比較](extensions.md)）[S24]／MCP（remote）✅ 免費帳號限 1 個自訂 connector [S9b][S12] | Free 即可 [S3][S15] | 問答、腦力激盪、潤飾文字 |
| **Claude Desktop**（Chat 分頁） | 本機安裝的殼，Chat 分頁內容仍跑在雲端 [S7] | ❌ Chat 分頁本身不行 [S7] | ⚠️ 同 Chat，雲端沙盒 [S7] | 同 Claude Chat，另可加本機 MCP（desktop extensions）[S13] | Free 可用 Chat 分頁；Cowork／Code 分頁要付費方案 [S3][S7] | 一個 App 內同時裝 Chat／Cowork／Code |
| **Cowork**（Desktop 第三分頁／web／mobile beta） | 預設是雲端 VM，既有 Desktop 部署也可能用本機專用 Linux VM [X4] | ✅ 本機或雲端 VM 內讀寫，需 Claude Desktop 保持開啟連線 [S21][X4] | ✅ 隔離 VM 內可跑 shell／程式碼，但不等於主機終端，不能叫它直接跑主機上的 `codex exec` [X4] | Skill✅／Plugin✅（可包含在主機執行的 local MCP server）／MCP（remote connector）✅ [S9b][X4] | Pro 起，免付費帳號完全不可用 [S21] | 整理檔案、做報表、寄信、跨應用文書工作 |
| **Claude Code**（CLI／Desktop Code 分頁／網頁／IDE） | CLI／IDE 擴充／Desktop 本機；claude.ai/code 是 Anthropic 管理的雲端 VM [S7][S8] | ✅ CLI／IDE／Desktop 可讀寫本機；網頁版是雲端 VM 內的 repo，不是使用者本機磁碟 [S7][S8] | ✅ 本機終端指令（CLI／IDE／Desktop）[S1] | Skill✅／Plugin✅／MCP（本機＋remote）✅／Subagent✅／Hooks✅ [S9b][S17][S18][S19][S20] | Free 沒有；Pro 起，也能純用 API key 按量付費、不吃訂閱額度 [S1][S23] | 跨檔批次處理、程式開發、可重跑流程 |
| **Claude Design**（canvas 設計工具） | ？查無：官方說明沒提到跑在本機還是雲端 [S22] | ？查無 [S22] | ？查無 [S22] | ？查無：官方說明沒列出 Skill／Plugin／MCP 支援 [S22] | Free 未提及；Pro 起（research preview）；Enterprise 預設關閉，需管理員手動開啟 [S22] | 對著畫面拖拉調整產出設計稿、原型、投影片、one-pager，而不是純寫 prompt [S22] |
| **ChatGPT**（網頁／手機） | OpenAI 雲端伺服器 [O10] | ❌ 不能 [O10] | ？查無：本次查證沒有涵蓋 ChatGPT 網頁版程式碼執行工具的細節 | Connectors／Apps✅（Free 僅內建 App，Plus 起可加自訂 MCP）[O22] | Free 即可 [A2 §1] | 問答、寫作、Deep Research |
...
### 4. Codex 桌面 App 已經併入 ChatGPT Desktop（2026-07-09）
